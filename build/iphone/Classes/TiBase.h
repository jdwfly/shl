...
// in simulator we redefine to format for shl Developer console
...
{\
	NSString * exceptionName = [@"org.shl." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.shl";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIshlNativeItemNone = -1, 
	UIshlNativeItemSpinner = -2,
	UIshlNativeItemProgressBar = -3,
	
	UIshlNativeItemSlider = -4,
	UIshlNativeItemSwitch = -5,
	UIshlNativeItemMultiButton = -6,
	UIshlNativeItemSegmented = -7,
	
	UIshlNativeItemTextView = -8,
	UIshlNativeItemTextField = -9,
	UIshlNativeItemSearchBar = -10,
	
	UIshlNativeItemPicker = -11,
	UIshlNativeItemDatePicker = -12,
	
	UIshlNativeItemInfoLight = -13,
	UIshlNativeItemInfoDark = -14,
	
	UIshlNativeItemDisclosure = -15,
	
	UIshlNativeItemContactAdd = -16
};
